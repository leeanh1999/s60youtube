...
/**
 * Phan han dung cua mot cookie muon song qua lan dong trinh duyet.
 *
 * Phai ghi CA HAI cach: `Max-Age` la cua RFC 2109, con Nokia Browser doi Symbian
 * theo kieu Netscape cu va chi doc `Expires`. Chi dat Max-Age thi may Nokia coi
 * day la cookie phien — thoat trinh duyet la mat, nguoi dung phai dang nhap lai
 * tu dau. Dat Expires truoc de may nao dung mot thuoc tinh la doc duoc ngay.
 */
function keepFor(seconds = ONE_YEAR) {
  const until = new Date(Date.now() + seconds * 1000).toUTCString();
  return `Path=/; Expires=${until}; Max-Age=${seconds}`;
}
...
function deviceCookie(id) {
  return `${DEVICE_COOKIE}=${id}; ${keepFor()}; HttpOnly; SameSite=Lax`;
}
...
/** Dat lai ma thiet bi cua chinh may dang goi, day han dung ra mot nam nua. */
function attach(res, deviceId) {
  if (!DEVICE_RE.test(String(deviceId || ''))) return;
  res.append('Set-Cookie', deviceCookie(deviceId));
}

/**
 * Nhan lai mot may cu qua dia chi ghi nho. Trinh duyet Symbian co may khi don
 * sach cookie luc thoat; mat ma thiet bi la may chu coi day la may la va bat
 * dang nhap lai tu dau, du file cookie cua nguoi ta van con nguyen tren dia.
 *
 * Ma thiet bi la 128 bit ngau nhien nen chinh no la chia khoa: ai co dia chi ay
 * thi xem duoc bang tai khoan do, hoan toan giong nhu ai cam duoc cookie cua
 * may. Vi vay dia chi nay chi hien tren trang Dang nhap cua chinh may do.
 */
function claim(res, deviceId) {
...
/**
 * Cookie de phat duoc video chua han du de mo trang rieng cua tai khoan (goi y,
 * kenh dang ky). Ban chi co phan '3P' van phat duoc, nhung YouTube coi cac
 * trang ngoi thu nhat la cua nguoi chua dang nhap. Ba cai duoi day la dau hieu
 * cua mot phien that.
 */
const PAGE_NAMES = ['SID', '__Secure-1PSID', 'LOGIN_INFO'];
...
/**
 * true = cookie du de mo trang rieng, false = thieu, null = khong biet (may
 * chua co cookie, hoac dang doc cookie thang tu trinh duyet cua may chu).
 */
function hasPageSession(deviceId) {
  const auth = authFor(deviceId);
  let file = null;
  if (auth.mode === 'device') file = fileFor(deviceId);
  else if (auth.mode === 'shared') file = config.COOKIES_FILE;
  if (!file) return null;
  try {
    const names = namesIn(file);
    return PAGE_NAMES.some((name) => names.has(name));
  } catch {
    return null;
  }
}
