 ...
 * Symbian khong doc duoc vo MP4 phan manh cua duong ghep — nen ai duoc thay
 * muc 'ghep' la viec cua noi goi ham nay (xem tham so remux).
 */
function streamChoices(formats, { remux = false } = {}) {
  ...
  if (remux && isBelleAudio(pickAudioOnly(formats))) {
    for (const format of formats) {
      ...
      const seen = byHeight.get(format.height);
      // Ban gop san o cung do phan giai luon hon: nhe hon va tua duoc.
      if (seen && seen.kind === 'san') continue;
      if (!seen || format.tbr > seen.tbr) {
        byHeight.set(format.height, {
          height: format.height,
          kind: 'ghep',
