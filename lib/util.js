...
/**
 * Dau thap phan la dau phay, cho khop voi chu YouTube tu gui ve ("1,8 Tr luot
 * xem") — hai kieu dung chung mot trang thi nhin ra ngay la lech.
 */
function short(n, unit, suffix) {
  const text = (n / unit)
    .toFixed(1)
    .replace(/\.0$/, '')
    .replace('.', ',');
...
function formatCount(value) {
  ...
  if (n >= 1e9) return short(n, 1e9, 'Tỉ');
  if (n >= 1e6) return short(n, 1e6, 'Tr');
  if (n >= 1e3) return short(n, 1e3, 'N');
  return String(n);
}
...
/** Doc header Cookie cua trinh duyet thanh object { ten: gia-tri }. */
