...
/**
 * Cau hinh chuyen ma cho tung doi may.
 * H.264 Baseline + AAC-LC la thu ma trinh phat Symbian doc duoc chac chan nhat:
 * khong B-frame, 1 reference frame, yuv420p.
 */
const PROFILES = {
  qcif: {
    label: '176x144 — máy S60v3 (N70, N73, E71)',
    ext: 'mp4',
    maxHeight: 240,
    video: ['-vf', 'scale=176:-2', '-r', '15', '-b:v', '160k', '-level', '1.3'],
    audio: ['-c:a', 'aac', '-b:a', '32k', '-ac', '1', '-ar', '22050'],
  },
  qvga: {
    label: '320x240 — máy S60v5 (5800, N97, X6)',
    ext: 'mp4',
    maxHeight: 360,
    video: ['-vf', 'scale=320:-2', '-r', '20', '-b:v', '350k', '-level', '2.1'],
    audio: ['-c:a', 'aac', '-b:a', '64k', '-ac', '2', '-ar', '44100'],
  },
  nhd: {
    label: '640x360 — Symbian Belle (N8, 808, E7, 701)',
    ext: 'mp4',
    maxHeight: 480,
...
function resolveFfmpeg() {
  try {
    ...
    // khong co goi ffmpeg-static, thu tiep PATH
  }
  const local = path.join(config.ROOT, 'bin', process.platform === 'win32' ? 'ffmpeg.exe' : 'ffmpeg');
