...
const PROFILES = {
  ...
  // Cung mot viec ghep nhu duong /hd, khac cho la ghep ra file that roi moi
  // phat: cho vai giay, doi lai co chi muc cho ca file nen tua duoc. Duong /hd
  // phat ngay nhung khong tua duoc, nen hai muc nay bu cho nhau.
  //
  // Chi hien cho may doi moi (xem isLegacyDevice trong server.js): man hinh
  // 640x480 cua Symbian khong dung toi 720p.
  hd: {
    label: 'Tạo bản 720p tua được',
    ext: 'mp4',
    mux: 'mp4',
    maxHeight: 720,
    modernOnly: true,
...
/**
 * Ghep mot luong hinh voi mot luong tieng roi tra thang ra duong truyen, khong
 * qua file nao. Dung cho may doi moi muon xem o do phan giai cao: YouTube nay
 * hiem khi con giu ban gop san tren 360p, ma cho ghep xong ca file roi moi xem
 * duoc thi mat han cai le "bam la chay".
 *
 * '-c copy' nen day chi la chep du lieu chu khong ma hoa: NAS chip ARM van kip.
 *
 * Vo MP4 phai la ban phan manh (fragmented): vo MP4 thuong de chi muc o cuoi
 * roi quay lai dau file ghi vao, ma duong truyen thi khong quay lai duoc. Voi
 * 'frag_keyframe+empty_moov' thi ffmpeg ghi tung manh mot, byte dau tien ra
 * ngay. Doi lai la khong co chi muc cho ca file nen trinh duyet khong tua
 * duoc — muon tua thi phai qua duong tao ban san.
 */
function remuxStream(videoUrl, audioUrl) {
  const args = ['-hide_banner', '-loglevel', 'error'];
  for (const url of [videoUrl, audioUrl]) args.push(...inputArgs(url));
  args.push(
    '-map',
    '0:v:0',
    '-map',
