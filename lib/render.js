...
<td class="br">${
    back
      ...
      : ''
  }<a class="find" href="/search" accesskey="*" title="Tìm">${icon('search')}</a></td>
</tr></table>
</div>
<div id="pop">
${searchForm(query)}
...
function rememberPage({ prefs }) {
  const body = `<div class="head"><h1>${icon('check')}<span>Đã ghi nhớ máy này</span></h1></div>
<p class="ok">${icon('check')}<span>Máy chủ vừa nhận lại tài khoản cho máy này.</span></p>
