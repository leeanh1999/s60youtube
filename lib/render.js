...
/**
 * Symbian^3 tro len (Nokia Browser 7.x va 8.x) — ban WebKit hieu HTML5, SVG noi
 ...
 *
 * O tim kiem thu gon thanh bieu tuong kinh lup o goc phai thanh tren. Co
 * JavaScript thi bam vao do mo ra khung tim kiem dan ngay duoi thanh; khong co
 * thi no la lien ket thuong sang /search — trang do van co o nhap that.
 *
...
<div class="bar">
<a class="find" href="/search" accesskey="*">${icon('search')}<span>Tìm</span></a>${
  back
...
function homePage({ warning, note, videos, prefs }) {
  ...
  const topics = TOPICS.map((topic, i) => {
    ...
    return `<li><a href="/search?q=${encodeURIComponent(topic.q)}"${
      key ? ` accesskey="${key}"` : ''
    }>${icon(topic.icon)}${key ? `<span class="k">${key}</span>` : ''}<span class="lb">${escapeHtml(
      topic.q
    )}</span></a></li>`;
  }).join('\n');
...
${note ? `<p class="warn">${icon('warn')}<span>${escapeHtml(note)}</span></p>` : ''}
...
function searchPage({ query, videos, prefs, nextPage, warning }) {
  const next = nextPage
    ...
    : '';
  // O nhap giu lai o day, khong thu thanh bieu tuong nhu cac trang khac: day la
  // cho may khong bat JavaScript go tu khoa, va la cho sua lai tu khoa vua tim.
...
${
  query
    ...
    : '<p class="hint">Gõ từ khoá rồi bấm Tìm. Dán link YouTube vào đây là mở đúng video đó.</p>'
}
${next}`;
