...
function layout({ title, body, back, prefs, refreshSeconds, refreshUrl, active, query }) {
  if (prefs?.mini) {
    return miniLayout({ title, body, back, prefs, refreshUrl, active, query });
  }
...
/**
 * Khung trang cho Opera Mini. Cung mot bo noi dung, cung ten lop, chi khac cach
 * bay ra — vi may chu cua Opera ve trang ho roi gui xuong dien thoai mot anh
 * chup bam duoc, chu dien thoai khong ve gi ca (xem isOperaMini trong
 * server.js). Bon cho khac:
 *
 *  1. KHONG NAP s60.js. Ca ba viec cua no — dan hai thanh vao khung nhin, mo
 *     khung tim kiem, bat phim len/xuong — deu can JavaScript chay luc nguoi
 *     dung bam, ma tren Opera Mini thi script dung han truoc khi trang toi
 *     duoc may. Nap vao chi ton mot lan tai file de khong chay gi.
 *
 *  2. O TIM KIEM NAM SAN trong moi trang thay cho cai kinh lup mo ra bang
 *     JavaScript. Khong co JavaScript thi kinh lup la duong sang /search — tuc
 *     la moi lan tim la mot lan goi qua proxy cua Opera, vai giay tren 2G. Mot
 *     dong o nhap re hon nhieu.
 *
 *  3. LAM MOI PHAI BAM. Opera Mini khong theo meta refresh: trang da la anh
 *     chup thi khong co gi chay tiep de ma dem gio. Trang nao dang doi viec
 *     (dang nhap, chuyen ma) thi duong lam moi phai la mot lien ket that, dat
 *     ngay dau trang.
 *
 *  4. KHONG CO PHIM TAT. accesskey va o so 1-9 chi lam nguoi dung bam thu roi
 *     tuong may hong, nen bo han — xem miniVideoRow va optionList.
 *
 * Chan trang cung khong con la bang bon o: tren man 128 diem moi o chi con 30
 * diem, nhan nao cung gay lam ba dong. Mot dong chu ngan cach bang dau cham
 * giua thi be ngang nao cung xuong dong dung cho.
 */
function miniLayout({ title, body, back, prefs, refreshUrl, active, query }) {
  const textSize = TEXT_SIZES[prefs?.textSize] ? prefs.textSize : DEFAULT_TEXT_SIZE;
  const tab = (href, label, id) =>
    active === id
      ? `<b class="on">${escapeHtml(label)}</b>`
...
<link rel="stylesheet" type="text/css" href="/mini.css?v=${ASSET_TAG}" />
</head>
<body class="t-${textSize}">
<div class="bar"><a class="brand" href="/">${icon('logo', 'white')}<span>YouTube S60</span></a>${
    back ? `<a class="back" href="${escapeHtml(back)}">Quay lại</a>` : ''
  }</div>
${searchForm(query, true)}
${
  refreshUrl
    ? `<p class="more"><a href="${escapeHtml(refreshUrl)}">${icon(
        'refresh',
        'blue'
      )}<span>Làm mới trang</span></a></p>`
    : ''
}
<div id="main">
${body}
...
<p class="nav">${[
    tab('/', 'Trang chính', 'home'),
    tab('/login', 'Đăng nhập', 'login'),
    tab('/settings', 'Cài đặt', 'settings'),
    tab('/about', 'Giới thiệu', 'about'),
  ].join(' · ')}</p>
</body>
</html>`;
}
 ...
 */
function searchForm(query = '', mini = false) {
  // Opera Mini: bang de chia dung cot thi o day khong con viec gi lam (mot o
  // nhap va mot nut, ban ve nao cung xep vua mot dong), ma no lai la mot bang
  // — thu trang nay tranh tren man 128 diem. Hai the nam canh nhau la du.
  if (mini) {
    return `<form class="search" action="/search" method="get">
<input type="text" name="q" value="${escapeHtml(query)}" /> <button type="submit">Tìm</button>
</form>`;
  }
  return `<form class="search" action="/search" method="get">
...
function videoRow(video, index, prefs) {
  if (prefs.mini) return miniVideoRow(video, prefs);
...
/**
 * Cung mot the video, ve cho Opera Mini. Cung mot y — anh ben trai, chu ben
 * phai — nhung dung ba cach khac:
 *
 *  - Anh do bang DIEM ANH va neo bang float, khong phai bang 'em' va khoi neo
 *    goc. Ban ve cua Opera chon co chu theo may chu khong theo trang, nen mot
 *    khoang trong do bang 'em' de chua anh se khong khop voi cai anh nam trong
 *    do: hut mot chut la chu de len anh, thua mot chut la mat nua dong chu tren
 *    man 240 diem.
 *  - Thoi luong xuong dong thong tin chu khong dan len goc anh: dan chu len anh
 *    lai la mot khoi neo tuyet doi nua.
 *  - Khong o so, khong accesskey: Opera Mini danh phim so cho menu cua chinh no.
 */
function miniVideoRow(video, prefs) {
  const duration = video.durationText || formatDuration(video.duration);
  const meta = [video.author, duration, video.views, video.published]
    .filter(Boolean)
    .map((part) => escapeHtml(truncate(String(part), 40)))
    .join(' · ');
  const live = video.live ? '<b class="live-t">TRỰC TIẾP</b> · ' : '';
  // 80 diem: tren man 240 diem (may Java hay gap nhat) thi con lai chung 140
  // diem cho chu, du hai muoi chu mot dong. Anh be hon nua thi khong con nhin
  // ra canh gi, to hon thi ten video vo con hai ba chu moi dong. Man rong hon
  // 240 diem thi anh nho di trong khung chu khong be trang.
  const thumb = prefs.thumbs
    ? `<img src="/thumb/${video.id}" width="80" height="45" alt="" />`
    : '';

  return `<li><a class="card" href="/watch?v=${video.id}">${thumb}<span class="ti">${escapeHtml(
    truncate(video.title, 70)
  )}</span><span class="mt">${live}${meta}</span></a></li>`;
}
...
function homePage({ warning, note, videos, prefs }) {
  ...
  const topics = TOPICS.map((topic, i) => {
    // Opera Mini danh phim so cho menu cua chinh no, khong chuyen xuong trang:
    // bay o so ra la nguoi dung bam thu roi tuong may hong.
    const key = feed || prefs.mini ? null : i + 1;
...
${
  prefs.mini
    ? `<p class="hint">Gõ vào ô tìm ở đầu trang; dán thẳng link YouTube vào đó là mở
đúng video. Trang này đang ở khuôn dành cho Opera Mini: không có phím tắt và
không tự làm mới, vì máy chỉ nhận một ảnh chụp trang từ máy chủ Opera.</p>`
...
function searchPage({ query, videos, prefs, nextPage, warning }) {
  const next = nextPage
    ? `<p class="more"><a href="/search?q=${encodeURIComponent(query)}&amp;p=${nextPage}"${
        // Opera Mini danh phim cho menu cua chinh no, khong chuyen xuong trang.
        prefs.mini ? '' : ' accesskey="#"'
      }>${icon(
        'next',
        'blue'
...
${
  query
    ...
    : // Opera Mini da co o nhap nam san o dau moi trang, khong dat them cai thu hai.
      `${prefs.mini ? '' : `${searchForm()}\n`}<p class="hint">Gõ từ khoá rồi bấm ${
        prefs.mini ? 'nút Tìm ở đầu trang' : 'kính lúp'
      }. Dán link YouTube vào đây là mở đúng video đó.</p>`
}
${next}`;
 ...
 */
function playerBox(src, label, poster, mse, mini) {
  // Opera Mini khong phat duoc gi trong trang: cai may nhan duoc chi la anh
  // chup, khong phai mot trinh duyet dang chay. Dat the <video> vao day thi tot
  // nhat cung chi ra duoc lien ket du phong ben trong no, con te hon la mot
  // khung den. Nen o day la mot nut bam thang toi file — dien thoai giao no cho
  // trinh phat cua may.
  if (mini) {
    return `<p class="play"><a class="go" href="${src}">${icon(
      'play',
      'white'
    )}<span>${escapeHtml(label)}</span></a></p>`;
  }
...
function optionList(items, mini = false) {
  const rows = items.map((item, i) => {
    if (!item.href) return `<li class="m">${escapeHtml(item.label)}</li>`;
    // Opera Mini danh phim so cho menu cua chinh no: o so va accesskey o day
    // chi la loi hua khong giu duoc.
    const key = i < 9 && !mini ? ` accesskey="${i + 1}"` : '';
    ...
    return `<li><a href="${item.href}"${key}>${item.icon ? icon(item.icon, 'red') : ''}${
      i < 9 && !mini ? `<span class="k">${i + 1}</span>` : ''
...
function playOptions({
  ...
  if (chosen) {
    parts.push(
      playerBox(chosen.src, 'Mở bằng trình phát của máy', `/thumb/${video.id}`, mse, mini)
    );
    ...
    parts.push(
      mini
        ? // Nut o tren khong phai khung phat ma la mot lien ket toi file: noi ro
          // chuyen gi sap xay ra, vi cai mo len la trinh phat cua may chu khong
          // phai trinh duyet, va duong di cua no khong qua may chu Opera nua.
          `<p class="hint">Opera Mini không phát được video ngay trong trang. Bấm nút trên
là điện thoại giao file cho trình phát của máy — máy phải tự nối thẳng tới máy chủ
này được thì mới chạy. Máy nào không phát nổi thì chọn <b>Nghe ngay — chỉ tiếng</b>
ở dưới, nhẹ hơn nhiều.</p>`
        : chosen.kind === 'ghep'
...
hoặc tạo bản ở dưới.</p>`
          : `<p class="hint">Xem online${
              chosen.height ? ` ${chosen.height}p` : ''
            } — máy chỉ tải phần đang xem, không lưu vào bộ nhớ.</p>`
    );
    // Tren Opera Mini nut o tren da chinh la lien ket nay roi, khong bay ra hai lan.
    if (!mini) {
      items.push({
        href: chosen.src,
        icon: 'screen',
  ...
  // Chua co danh sach luong thi chua biet video nay co luong tieng AAC hay
  // khong. Cu bay muc do ra: hau nhu video nao cung co, va duong /audio tu tim
  // lay luc bam vao — sai thi mat mot lan bam, con giau di thi mat han mot
  // duong nghe cho may 2G.
  if (!ready || info?.audioDirect) {
    items.push({
  ...
  // May doi moi con co cac muc 720p/1080p, nhung phai co danh sach luong moi
  // biet video nay den muc nao. Luc mo trang lai thi yt-dlp da xong tu doi nao.
  if (!ready && !legacy) {
    items.push({
      href: `/watch?v=${video.id}`,
      icon: 'screen',
      label: 'Xem mức nét hơn (720p, 1080p)',
      note: 'Mở lại trang này khi máy chủ đã hỏi xong YouTube.',
    });
  }
  ...
  if (!ffmpegOk) {
    ...
    for (const [id, profile] of Object.entries(profiles)) {
      ...
      if (profile.audioOnly && (!ready || info?.audioDirect)) continue;
  ...
  parts.push(optionList(items, mini));
...
function watchPage({
  ...
  // Chua co ket qua yt-dlp thi phan chu lay tu InnerTube (xem server.js).
  const text = info?.description || video.description;
  const description = text
    ? `${section('Mô tả', 'info')}
<div class="desc">${escapeHtml(truncate(text, 500)).replace(/\n/g, '<br />')}</div>`
    : '';
...
${
  profile.audioOnly
    ? ''
    : playerBox(file, 'Mở bằng trình phát của máy', `/thumb/${job.videoId}`, null, prefs.mini)
}
${optionList(
  [
    // Tren Opera Mini khung phat da la chinh lien ket nay; ban chi tieng thi
    // khong co khung nao nen van phai bay ra.
    prefs.mini && !profile.audioOnly
      ? null
      : {
          href: file,
          icon: profile.audioOnly ? 'music' : 'screen',
          label: 'Mở bằng trình phát của máy',
        },
...
<p class="hint">${
    prefs.mini
      ? 'Opera Mini không tự làm mới được: bấm <b>Làm mới trang</b> ở đầu trang sau vài giây để xem đã xong chưa.'
      : 'Trang tự làm mới sau vài giây.'
  } Video dài thì chờ lâu hơn.</p>
${optionList(
  [
    {
...
${
  // Ma QR di qua Opera Mini la mot anh bi nen lai lan nua cho nhe duong truyen:
  // hinh chu no nho tung o vuong, nen lam nhoe di la may khac soi khong ra. Ma
  // chu thi go tay van dung. Bo luon ca mot lan tai anh 200x200.
  prefs.mini
    ? ''
    : `<p class="hint">Hoặc quét mã vạch bằng máy khác:</p>
<p class="qr"><img src="/qr?c=${encodeURIComponent(
        code
      )}" width="200" height="200" alt="Mã QR" /></p>`
}
<p class="hint">Mã còn dùng được ${minutes} phút và chỉ nạp cookie vào đúng máy
này. ${
    prefs.mini
      ? 'Nộp cookie bên kia xong thì bấm <b>Làm mới trang</b> ở đầu trang — Opera Mini không tự làm mới được.'
      : 'Trang tự làm mới, xong bên kia là ở đây báo ngay.'
  }</p>
...
${error ? `<p class="err">${icon('warn', 'red')}<span>${escapeHtml(error)}</span></p>` : ''}
${optionList([{ href: '/login?new=1', icon: 'refresh', label: 'Lấy mã mới' }], prefs.mini)}`;
...
<div class="m">${
    prefs.mini
      ? 'Opera Mini không lấy đúng cỡ chữ trang xin mà chọn trong vài cỡ có sẵn của' +
        ' chính điện thoại, nên mục này có khi đổi rất ít hoặc không đổi gì. Cỡ chữ' +
        ' thật nằm trong phần Cài đặt của Opera Mini.'
      : 'Màn hình nhỏ mà dày điểm ảnh, chữ mặc định của web hiện ra rất bé. Trang tự' +
        ' hạ một bậc khi máy xem dọc (N8) vì bề ngang chỉ còn một nửa.'
  }</div>
</div>
...
${
  // Qua Opera Mini may chu tu ha xuong toi da 8, nen bay ra nhung con so lon hon
  // la de nguoi dung chon mot thu khong bao gio thanh — va chon xong lai thay
  // dung so cu.
  (prefs.mini ? [4, 6, 8] : [6, 10, 12, 20, 30])
    .map((n) => option(n, String(n), prefs.pageSize))
    .join('')
}
</select>
${
  prefs.mini
    ? '<div class="m">Qua Opera Mini thì nhiều nhất là 8: cả trang đi xuống máy dưới' +
      ' dạng một ảnh chụp, dài quá là Opera cắt ra làm nhiều phần và mỗi phần lại là' +
      ' một lần chờ.</div>'
    : ''
}
</div>
...
${
  // Phim tat la viec cua s60.js, ma tren Opera Mini thi khong co script nao
  // chay trong may: bay bang phim ra day chi la loi hua khong giu duoc.
  prefs.mini
    ? `${section('Đang xem bằng Opera Mini', 'info')}
<p class="hint">Máy chủ nhận ra Opera Mini nên trang này đang ở khuôn riêng: bố cục
đơn giản hơn, ô tìm nằm sẵn ở đầu trang, không phím tắt và không tự làm mới. Trang
được vẽ trên máy chủ của Opera rồi gửi xuống máy dưới dạng một ảnh chụp bấm được,
nên mọi thứ cần chạy trong máy đều không có. Video thì không phát trong trang được:
bấm nút phát là điện thoại giao cho trình phát của máy.</p>`
...
${optionList([{ href: escapeHtml(back || '/'), icon: 'back', label: 'Quay lại' }], prefs?.mini)}`;
