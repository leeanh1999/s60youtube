...
function playOptions({ video, info, choices, chosen, profiles, ffmpegOk, streamKey, legacy }) {
  const parts = [];
  const items = [];

  if (chosen) {
    parts.push(playerBox(chosen.src, 'Mở bằng trình phát của máy', `/thumb/${video.id}`));
    if (choices.length > 1) parts.push(qualityRow(choices, chosen));
    // Ban ghep khong co chi muc cho ca file nen khong tua duoc — phai noi ra
    // truoc, khong thi nguoi dung tuong may hong khi keo thanh thoi gian.
    parts.push(
      chosen.kind === 'ghep'
        ? `<p class="hint">Xem thẳng ${chosen.height}p — máy chủ ghép hình với tiếng ngay lúc
...
${
  error
    ...
    : playOptions({ video, info, choices, chosen, profiles, ffmpegOk, streamKey, legacy })
}
${description}
