...
/**
 * Do phan giai toi da khi xem online. Man hinh E6 la 640x480 va luong gop san
 * cao nhat cua YouTube nam duoi muc do la 360p (itag 18) — do la mac dinh, may
 * Symbian nao cung mo thang duoc.
 *
 * Cung dia chi nay con duoc mo bang may doi moi (dien thoai Android, trinh
 * duyet may tinh) — nhung may do doc duoc ban 720p gop san cua YouTube, nen cho
 * chon. Chon cao khong lam hong may yeu: khong co ban nao vua muc da chon thi
 * lay ban cao nhat con lai.
 */
const VIDEO_HEIGHTS = {
  360: '360p — mọi máy Symbian mở được',
  480: '480p — máy khá, mạng nhà',
  720: '720p — máy đời mới, mạng khoẻ',
};
...
function playOptions({ video, info, profiles, ffmpegOk, streamKey, prefs }) {
  const parts = [];
  const items = [];
  const height = VIDEO_HEIGHTS[prefs?.maxHeight] ? prefs.maxHeight : DEFAULT_VIDEO_HEIGHT;
  // Trinh phat cua may mo lien ket nay ben ngoai trinh duyet nen khong gui
  // cookie theo — ma do phan giai la mot tuy chon nam trong cookie. Nen ca hai
  // deu phai di trong chinh dia chi: khoa de may chu biet dung cookie cua ai,
  // va muc do phan giai da chon.
  const query = [
    streamKey ? `k=${encodeURIComponent(streamKey)}` : '',
    height === DEFAULT_VIDEO_HEIGHT ? '' : `h=${height}`,
  ]
    .filter(Boolean)
    .join('&amp;');
  const stream = `/stream/${video.id}${query ? `?${query}` : ''}`;

  if (info?.direct) {
    // Khong phai video nao YouTube cung con ban gop san tren 360p. Noi ro chu
    // khong de nguoi dung ngo minh chon 720p ma trang lang le cho xem 360p.
    const lower =
      info.direct.height < height
        ? ` YouTube không có sẵn bản ${height}p ghép sẵn cho video này.`
        : '';
    parts.push(playerBox(stream, 'Mở bằng trình phát của máy', `/thumb/${video.id}`));
    parts.push(
      `<p class="hint">Xem online ${info.direct.height}p — máy chỉ tải phần đang xem, không lưu vào bộ nhớ.${lower}</p>`
    );
    items.push({
      href: stream,
      icon: 'screen',
  ...
  if (!ffmpegOk) {
    ...
    for (const [id, profile] of Object.entries(profiles)) {
      ...
      // Ban do phan giai cao chi hien khi nguoi dung da chon xem o muc do:
      // may Symbian de mac dinh 360p thi khong phai luot qua mot muc vo dung.
      if (profile.needsHeight && height < profile.needsHeight) continue;
      items.push({
...
${
  error
    ...
    : playOptions({ video, info, profiles, ffmpegOk, streamKey, prefs })
}
${description}
...
<div class="row"><label for="vh">Độ phân giải khi xem online</label>
<select id="vh" name="maxHeight">
${Object.entries(VIDEO_HEIGHTS)
  .map(([value, label]) =>
    option(value, label, String(prefs.maxHeight || DEFAULT_VIDEO_HEIGHT))
  )
  .join('')}
</select>
<div class="m">Máy Symbian mở chắc chắn bản 360p, để nguyên mức đó là chạy được ở
mọi máy. Mở cùng địa chỉ này bằng điện thoại đời mới hay máy tính thì chọn cao hơn
cho nét. Video nào YouTube không còn bản ghép sẵn ở mức đã chọn thì trang lấy bản
cao nhất còn lại — hoặc nhờ máy chủ ghép ở trang video.</div>
</div>
